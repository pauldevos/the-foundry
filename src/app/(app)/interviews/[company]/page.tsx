...
export default async function CompanyInterviewPage({
  ...
  if (!data) {
    return (
      <div>
        <Link href="/interviews" className="mb-6 inline-block text-sm text-stone-500 hover:text-amber-500">
          ← Interviews
        </Link>
        <div className="rounded-lg border border-stone-800 bg-stone-900 p-8 text-center">
          <p className="font-serif text-xl text-stone-100">Prep coming soon</p>
  ...
  return (
    <div>
      <Link href="/interviews" className="mb-4 inline-block text-sm text-stone-500 hover:text-amber-500">
        ← Interviews
      </Link>
      ...
      <div className="mb-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="font-serif text-2xl text-stone-100">{data.name}</h1>
        ...
        <p className="mt-4 text-sm leading-relaxed text-stone-300">{data.overview}</p>
      </div>

      {/* Architecture */}
      <section className="mb-8">
        <h2 className="mb-1 font-mono text-xs uppercase tracking-wide text-amber-500">
          {data.architecture.title}
        </h2>
        <p className="mb-4 text-sm text-stone-400">{data.architecture.problem}</p>
        <div className="space-y-2">
          {data.architecture.flow.map((item) => (
            <div
              key={item.step}
              className="flex gap-3 rounded-lg border border-stone-800 bg-stone-900 p-3"
            >
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-stone-400">{item.detail}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Databricks Services */}
      <section className="mb-8">
        <h2 className="mb-3 font-mono text-xs uppercase tracking-wide text-amber-500">
        ...
        <div className="space-y-2">
          {data.databricksServices.map((svc) => (
            ...
            >
              <p className="text-sm font-medium text-stone-100">{svc.name}</p>
              <p className="mt-0.5 text-xs text-stone-300">{svc.role}</p>
      ...
      <section className="mb-8">
        ...
        <div className="space-y-3">
          {data.interviewAngles.map((qa, i) => (
            <details key={i} className="rounded-lg border border-stone-800 bg-stone-900">
              <summary className="cursor-pointer p-4 text-sm text-stone-200 hover:text-amber-400">
                {qa.question}
              </summary>
              <p className="border-t border-stone-800 px-4 pb-4 pt-3 text-sm leading-relaxed text-stone-300">
                {qa.angle}
              </p>
            </details>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {data.studyMaterials.map((m) => (
            <Link
              key={m.label}
              href={m.href}
              className="rounded-lg border border-stone-700 bg-stone-900 px-3 py-2 text-sm text-stone-300 hover:border-amber-700 hover:text-amber-400"
            >
              {m.label}
            </Link>
          ))}
