...
const statusColor: Record<string, string> = {
  ...
  "Prep Phase": "border-amber-800 text-amber-400",
};

export default function InterviewsPage() {
  return (
    <div>
      <h1 className="mb-2 font-serif text-2xl text-slate-200">Interviews</h1>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="font-serif text-slate-200">{co.name}</p>
                <p className="mt-0.5 text-xs text-slate-400">
                  {co.vertical} · {co.role}
                </p>
                <p className="mt-2 text-sm text-slate-300">{co.context}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {co.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full border border-stone-700 px-2 py-0.5 font-mono text-xs text-slate-400"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <span
                className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-medium ${statusColor[co.status] ?? "border-stone-700 text-slate-400"}`}
              >
                {co.status}
              </span>
            </div>
