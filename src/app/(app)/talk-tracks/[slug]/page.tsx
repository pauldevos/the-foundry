...
export default async function TalkTrackDomainPage({
  ...
  return (
    <div>
      <h1 className="mb-1 font-serif text-2xl text-stone-100">{slug}</h1>
      ...
      <div className="space-y-6">
        {sections.map(({ s, state }) => {
          ...
          return (
            <div key={s.key} className="rounded-lg border border-stone-800 bg-stone-900 p-4">
              <h3 className="mb-2 font-serif text-lg text-stone-100">{s.heading}</h3>
              {director || staff ? (
                <>
                  {director && (
                    <div className="mb-3">
                      <p className="mb-1 font-mono text-xs uppercase tracking-wide text-amber-500">
                        Director
                      </p>
                      <p className="text-sm leading-relaxed text-stone-300">{director}</p>
                    </div>
                  )}
                  {staff && (
                    <div>
                      ...
                      <p className="text-sm leading-relaxed text-stone-300">{staff}</p>
                    </div>
                  )}
                </>
              ) : (
                <p className="text-sm leading-relaxed text-stone-300">{s.unified}</p>
              )}
              <TalkTrackActions
                id={encodeKey(s.key)}
                director={director}
